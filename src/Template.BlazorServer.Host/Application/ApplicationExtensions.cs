...
public static class ApplicationExtensions
{
    ...
    public static IHostApplicationBuilder ConfigureAuthentication(this IHostApplicationBuilder builder)
    {
        ...
        // Identityストア(専用のDbContext+SQLite。業務データはSmart.Data.Accessorと併存)
        ...
        builder.Services
            .AddIdentityCore<ApplicationUser>(static options =>
            {
                options.SignIn.RequireConfirmedAccount = false;
                // パスキー(WebAuthn)対応スキーマ
        ...
        // ApplicationCookieの安全既定(ベースの自前Cookie認証と同等)
    ...
    public static async ValueTask InitializeApplicationAsync(this WebApplication app)
    {
        ...
        // Prepare identity store (EF Core Migrationで作成・更新。既存DBのモデル変更も反映される)
        ...
        if (await userManager.FindByNameAsync(setting.InitialId) is null)
        {
            var user = new ApplicationUser { UserName = setting.InitialId };
            var result = await userManager.CreateAsync(user, setting.InitialPassword);
            if (result.Succeeded)
            {
